...
def taskAnnexe(data):
    ...
    for file in glob.glob(f"output/Results/{dt}/*/*taskAnnexe.csv"):
        c = file.split('/')[3]
        if file.split('/')[2] == dt:
            cmd = f"python3 scripts/EvaluationClassification.py --references='input/evaluation/{dt}Annexe.csv' --predictions='{file}' --data='{dt}' --corpus='{c}'"
            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True)
            out, err = p.communicate() 
            result = out.split()
            for lin in result:
    ...
    for l in lignes:
        ...
        for config, infos in dic.items():
            for score_name, score in infos["score"].items():
                ...
                res[score_name].append([score, config, toto, infos['corpus']])
